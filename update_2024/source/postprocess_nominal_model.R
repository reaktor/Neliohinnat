...
# Run this in the "update_202x" directory

source("source/common.R")

d <- readRDS('data/d_20240127.rds') # Stan input, kind of
s <- readRDS("data/samples_2000+2000_20240131.rds")
...
traceplot(s, 'beta_year[1,2]', inc_warmup=F)
traceplot(s, 'beta_year[7,5]', inc_warmup=F)
traceplot(s, 'beta_year[3,9]', inc_warmup=F)

#mcmc_intervals(s, pars = vars(starts_with('pred_mean[2')))


res.long <- cbind(pnro = as.character(d$pnro), year = d$year, obs_price = d$price,
                 population = d$population.x,
                 n = d$n,
  ...
  dplyr::group_by(pnro, sample) %>%
  dplyr::mutate(delta = pred - lag(pred, default = first(pred), order_by = year)) %>%
  ungroup() %>%
  mutate(trend = delta/(pred - delta) ) %>% filter(pnro != '70210')

res <- res.long %>% filter(year == '2023') %>% group_by(pnro) %>%
  dplyr::summarise(trend2023_q25 = quantile(trend, 0.25),
                   trend2023_q75 = quantile(trend, 0.75),
                   price2023_q25 = quantile(pred, 0.25),
                   price2023_q75 = quantile(pred, 0.75),
                   price2023 = mean(pred),
                   trend2023 = mean(trend),
                   population = first(population)) %>%
  ungroup() %>%
  mutate(trend2023_reliable = (trend2023_q25*trend2023_q75 > 0))
...
saveRDS(res, "./data/pnro-hinnat_nominal_2023.rds")
saveRDS(predictions, "./data/predictions_nominal_2023.rds")
...
get.cov.betas <- function(d, s) {
  ...
  tmp = rstan::extract(s, pars="beta_year")[[1]] %>% reshape2::melt() %>% setNames(c("i", "year", "var", "beta")) %>% 
   ...
   geom_line(alpha=.05) + geom_hline(yintercept=0, color=I("red"), alpha=I(.5)) + facet_wrap(~ var, scales = 'free_y') +
   theme_bw(12)
  tmp
}
...
some_pnros <- c("00100", "59130", "00320", "02940", "76100", "02100", "49200")

predictions %>% 
  filter(pnro %in% some_pnros) %>% tidyr::gather(q, y, -pnro, -year,  -n_obs, -obs_price) %>% 
  #left_join(d %>% distinct(pnro, name)) %>% 
  { ggplot(.) + 
    geom_line(aes(x=year, y=y, color=q)) + 
    geom_point(data = . %>% distinct(year, pnro, n_obs, obs_price) %>% filter(!is.na(obs_price)), 
               aes(x=year, y=obs_price, size=n_obs, alpha=I(0.6))) +
    facet_wrap(~ pnro, scales="free_y") }
...
res %>% plyr::dlply("pnro", function (i) list(hinta2023=i$price2023, 
                                              trendi2023=i$trend2023,
                                              trendi2023_min=unname(i$trend2023_q25),
                                              trendi2023_max=unname(i$trend2023_q75))) %>% 
...
require('gridExtra')
load('./data/pnro_data_20240127.RData')
pnro2021 <- pnro.ashi.dat %>% filter(year == '2021') %>% 
  filter(!is.na(price)) %>% select(pnro, price)
res_short = res %>% mutate(type = 'orig') %>%
  left_join(pnro2021) %>%
  rbind(res %>% mutate(type = 'pred', price = price2021))
...
pnro_sf %>% left_join(res_short) %>% filter(!is.na(type)) %>%
  mutate(lprice = log(price2021),
         type = factor(type, levels = c('orig', 'pred'),
                       labels = c('Observed price for 2021', 'Prediction for 2021'))) %>%
