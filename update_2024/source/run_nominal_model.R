...
load("data/pnro_data_20240127.RData") # pnro.ashi.dat, pnro.population, pnro.sf

pnro.population = pnro.population %>% filter(population > 0) %>% arrange(pnro, year)

d_covs0 <- get_covariates(pnro.population, impute = T, include_intercept = T, level3_dummies = F)
d_covs0$year = pnro.population$year
d_covs <- d_covs0 

d <- d_covs %>% left_join(pnro.ashi.dat, by = c('pnro', 'year')) %>%
  mutate_history_vars() %>% mutate(pnro = as.factor(pnro))
rm(pnro.ashi.dat, pnro.population, pnro.sf)

STAN_INPUT <- "data/d_20240127.rds"
...
if (T) {
  ...
  saveRDS(s, "data/samples_100+100_20240128.rds")
  toc()
}
