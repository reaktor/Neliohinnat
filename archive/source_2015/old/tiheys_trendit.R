...
for (index in seq(length(pnro.sp.alt@data$pnro))) {
  if (pnro.sp.alt@data$pnro[index] %in% rows){
    ...
    if (length(kunta) > 0){
      if (population[pnro.sp.alt@data$pnro[index],]$vakiluku > 0 & kunta %in% isot) {
...
ggplot(population, aes(x=logtiheys,y=trendi)) + geom_point(shape=1,aes(color=area)) + geom_smooth(method="gam", formula = y ~ s(x))
#ggplot(population, aes(x=logtiheys,y=trendi,color=maakunta)) + geom_point(shape=1) + geom_smooth(method="rlm")
ggsave("trendi_tiheys_Tampere.png")
dev.off()
