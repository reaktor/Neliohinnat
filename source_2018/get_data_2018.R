...
library("pxweb")
# update_pxweb_apis()  # update if needed
...
# [statfin_ashi_pxt_004.px] (for 2017 and earlier: [040_ashi_tau_104.px])
# Vanhojen osakeasuntojen keskihinnat ja kauppojen lukumäärät postinumeroalueittain ja rakennusvuosittain
pnro.ashi.raw <- pxweb::get_pxweb_data(url = "http://pxnet2.stat.fi/PXWeb/api/v1/fi/StatFin/asu/ashi/statfin_ashi_pxt_004.px",
...
# [2018/paavo_9_koko_2018.px]
pnro.population.raw <- pxweb::get_pxweb_data(url = "http://pxnet2.stat.fi/PXWeb/api/v1/fi/Postinumeroalueittainen_avoin_tieto/2018/paavo_9_koko_2018.px",
...
# Save all data
save(pnro.dat, pnro.sp, pnro.ashi.dat, file="/data_2018/pnro_data_20180301.RData")
...
# Note: the latest topojson packages do not have topojson command, so use 1.x versions
# See: https://github.com/topojson/topojson/issues/315
...
# 2018: write first as shapefile and then convert to topojson
rgdal::writeOGR(pnro.sp, "temp_pnro_shape_2018", "pnro-rgdal", driver="ESRI Shapefile")
# Use https://github.com/mbostock/topojson/ to convert
# system("topojson -p pnro -o json_2018/pnro.topojson temp_pnro_shape_2018/pnro-rgdal.shp")
...
pnro.info.list %>%
  toJSON(pretty=FALSE) %>%
  writeLines(con="json_2018/pnro_info_nonpretty.json")
