...
parameters {
    ...
    vector[ncovs] beta_cov_yr;
    vector[ncovs] beta_cov_yr2;
}
transformed parameters {
    ...
    matrix [N, 3] X_z;
    matrix [N, 6] X;
    mean_beta = append_col(mean_beta_plain, mean_beta_cov);
    z = covs * beta_cov;
    z_yr = covs * beta_cov_yr;
    z_yr2 = covs * beta_cov_yr2;
    for (i in 1:N) { X_z[i, 1] = z[i]; X_z[i, 2] = yr[i]*z_yr[i]; X_z[i, 3] = yr[i]*yr[i]*z_yr2[i]; }
