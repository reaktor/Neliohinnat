...
d <- readRDS('data/d_20220426.rds') # Stan input, kind of
s <- readRDS("data/samples_5000+5000_20220426.rds")
...
res <- res.long %>% filter(year == '2021') %>% group_by(pnro) %>%
  dplyr::summarise(trend2021_q25 = quantile(trend, 0.25),
                   trend2021_q75 = quantile(trend, 0.75),
                   price2021_q25 = quantile(pred, 0.25),
                   price2021_q75 = quantile(pred, 0.75),
                   price2021 = mean(pred),
                   trend2021 = mean(trend),
                   population = first(population)) %>%
  ungroup() %>%
  mutate(trend2021_reliable = (trend2021_q25*trend2021_q75 > 0))
...
saveRDS(res, "/data/pnro-hinnat_nominal_2022.rds")
saveRDS(predictions, "/data/predictions_nominal_2022.rds")
...
res %>% plyr::dlply("pnro", function (i) list(hinta2021=i$price2021, 
                                              trendi2021=i$trend2021,
                                              trendi2021_min=unname(i$trend2021_q25),
                                              trendi2021_max=unname(i$trend2021_q75))) %>% 
