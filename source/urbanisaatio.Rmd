...
for (ind in seq(length(trends$trend.y.mean))) {
  if (ind%%1000==0){
    print (ind)}
  if (trends[ind,]$pnro %in% rownames(population)) {
    trends[ind,]$logtiheys = population[trends[ind,]$pnro,]$logtiheys
    trends[ind,]$kunta = population[trends[ind,]$pnro,]$kunta
    }
  else {
    print (c("postinumeroa ",trends[ind,]$pnro," ei löytynyt"))
  }
}
...
for (ind in seq(length(trends$kunta))) {
    if (!(trends$kunta[ind] %in% isot)) 
    {
            trends$kunta[ind] = "muu" 
    }
}  
